...
export const validatePostID = (url: string) => {
  ...
  if (url.indexOf('-') > -1) {
    ...
    if (split[split.length - 1].length < 32) {
      return false
    }
  }
...
export const extractPostTitleURL = (origin: string) => {
  const url = new URL(origin)
  let str = decodeURIComponent(url.pathname)
